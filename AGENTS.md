...
## Railway MCP boundary

- Railway administrative access is allowed only through the globally configured MCP server named `railway`. Never use Railway through direct CLI subcommands, HTTP/GraphQL APIs, `curl`, the dashboard/browser, environment tokens, SDKs, plugins, or another client. The only permitted Railway CLI process is `/opt/homebrew/bin/railway mcp proxy` when Codex launches it as the stdio transport for the `railway` MCP server; do not use the CLI for any other Railway operation.
- The required Codex MCP configuration is `command = "/opt/homebrew/bin/railway"`, `args = ["mcp", "proxy"]`, `required = true`, and `default_tools_approval_mode = "approve"`. This gives the MCP server access to all tools exposed by the authenticated Railway account without routine client approval prompts. If the server is missing, unauthenticated, fails to initialize, or lacks a required tool, stop and report the MCP problem; do not fall back to another Railway access path.
- At the start of Railway work, use the MCP `whoami` tool to verify the authenticated account, then target the canonical production resources by exact ID: workspace `f659a058-986f-4a97-bf33-3060837ac4d3`, project `aa2423af-32c8-4dc0-9129-3db69c7e4a5d`, production environment `ed9b7bff-19ed-4ff8-9b9f-ff159411c11a`, application service `1fb8e716-8ba7-4906-80fd-9226e0eeb43e`, and Prowlarr service `5c2d7142-a357-4f99-a8fb-bcf6f47fbee5`.
- Several Railway projects are named `Kheyflix`; never select a project by name, list position, creation date, or guesswork. Before every write, confirm that the resolved project, environment, and service IDs match the canonical IDs above.
- Full MCP tool approval does not authorize unrelated or destructive changes. Keep actions within the user's request, inspect current state before writes, and obtain explicit user confirmation for destructive tools such as service removal, domain deletion, storage removal, redeploy, `accept-deploy`, or `railway-agent` when it may mutate infrastructure.
- Railway MCP variable reads can return plaintext secrets. Request variable values only when required, never print them into logs or chat, and never write them to tracked files. Production application health and playback may still be verified through the public Kheyflix URL; all Railway control-plane status, configuration, logs, metrics, and deployment actions must remain MCP-only.
