...
1. **Exact-release first-frame integrity:** The exact `992380faefcaa3697433b30795331aca674b36a8` production workflow caught a false fast-start measurement: an Auto promotion could rotate a bootstrap source before its first decoded frame, then report only the replacement source's time. This branch keeps one viewer-journey clock, invalidates stale decoder callbacks, and records a bootstrap EOF before its first frame as recovery. The exact merged revision must pass the live playback gate with a clean first-source frame and a true under-ten-second viewer startup measurement.
