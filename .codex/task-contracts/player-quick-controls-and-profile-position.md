...
- 2026-08-24, descendant production `a651d9bb`: mouse overlay remained visible on all three attempts because unchanged-coordinate `mousemove` events reset the timeout during DOM/video transitions.
- 2026-08-24, pointer-motion candidate: unchanged pointer coordinates no longer reveal/reset controls; focused real-catalog regression passed 3/3 consecutive runs. Current-main media-sync integration tests remained locally provider/timing-blocked outside this UI diff; current-main CI had already passed that suite.
