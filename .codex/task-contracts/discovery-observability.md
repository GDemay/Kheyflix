...
- 2026-08-24, production `26050131`: health `ok`, required dependencies healthy, and Pokémon Movies returns zero results with zero FitGirl/non-video releases. Follow-up empty-state regression first failed on the ambiguous baseline copy, then passes 18/18 across phone, tablet, and laptop with explicit “No playable movie releases” guidance.
- 2026-08-24, main CI attempts 1–2: exact deployment wait and production verifier passed; the live Smiling Friends playback fixture intermittently exceeded its 10-second first-frame threshold or stopped advancing on phone/laptop, matching the pre-change intermittent production failure. No assertions were weakened. The follow-up preserves successful streaming responses and bodies by identity while adding correlation headers; an independent 206 probe and the focused observability suite pass.
