...
- 2026-08-25, final integration on `7c00900b`: reconciled the independently merged unobtrusive desktop-control behavior with collision-free player surfaces. Desktop active playback uses bottom chrome without central quick controls; touch keeps central quick controls; pause/video dimming and loader/error/iOS exclusivity remain shared. Lint PASS; production build PASS; full deterministic suite PASS (31 files / 186 tests). An earlier run exposed a local macOS provenance stall in the bundled Intel FFprobe binary; clearing that untracked dependency attribute restored the unchanged media-sync probes.
