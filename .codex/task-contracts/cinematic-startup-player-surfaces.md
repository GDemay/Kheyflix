...
| AC-6 | Existing seek, pause/play, settings, audio, subtitle, adaptive-quality, recovery, and continuously advancing playback behavior remain intact. | Existing unit/UI suites and real-catalog laptop/phone playback | independent pass |
...
- 2026-08-24, post-rebase gate on `origin/main` `26050131`: the first full run passed 178/179 with one known host-load FFprobe timeout; the unchanged media-sync file then passed 3/3 in isolation and the complete rerun passed 31 files / 179 tests. Lint and production build PASS.
- 2026-08-24, staging isolation defect: duplicating production carried its GitHub deployment trigger into the staging `kheyflix` instance; concurrent `main` deployments replaced the CLI candidate during playback verification. A source-free `kheyflix-staging` service (`2f853515`) was created in the same canonical project/environment, configured with isolated server-side variables, and made the only target of the pinned staging deploy helper.
- 2026-08-24, exact source-free candidate `0c8b9040`: startup normal/reduced-motion tests passed 4/4 on phone and laptop; `/api/health` reported `ok` with AllDebrid, discovery, and transcoder healthy; the live catalog exposed 690 records, 449 ready records, and 3,630 playable video files.
- 2026-08-24, exact staging real-media gate: the native H264 Shrek catalog path decoded in 2.144 seconds on phone and 3.009 seconds on laptop during the complete run, then continuously advanced through pause/video-only dimming, seek, and 480p quality selection. A separate real multilingual Shrek source verified English audio and subtitle controls. Result: 5 PASS / 1 intentional phone skip (mouse-only pointer behavior).
- 2026-08-24, Railway cleanup: redundant projects `e65db96b` and `fe696b30` are deleted; canonical project `aa2423af` is the only active Kheyflix project. The obsolete GitHub-triggered `kheyflix` instance was removed only from staging, the verified source-free service now owns `kheyflix-staging.up.railway.app`, and production remained healthy.
- 2026-08-25, independent Sol/light acceptance on candidate `fa1e330b`: AC-1 through AC-6 PASS; startup, surface exclusivity, and real Shrek playback passed with first decoded frames of 1.367 seconds on phone and 0.986 seconds on laptop. AC-7 remained pending because the candidate was not merged/production-deployed and Railway CLI health provenance is null.
- 2026-08-25, final integration on `709657be`: reconciled the independently merged unobtrusive desktop-control and single-stream iPhone HLS improvements with collision-free player surfaces. Desktop active playback uses bottom chrome without central quick controls; touch keeps central quick controls; pause/video dimming and loader/error/iOS exclusivity remain shared. Lint PASS; production build PASS; full deterministic suite PASS (31 files / 187 tests). An earlier run exposed a local macOS provenance stall in the bundled Intel FFprobe binary; clearing that untracked dependency attribute restored the unchanged media-sync probes.
