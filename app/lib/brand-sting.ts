...
const scheduleKheyflixSting = (context: AudioContext) => {
  ...
  setTimeout(() => closeContext(context), 2500);
};
...
export function playKheyflixSting() {
  ...
  try {
    ...
    const start = () => {
      ...
      scheduleKheyflixSting(context);
    };
    function unlock() {
      if (started || disposed || unlocking) return;
      unlocking = true;
    ...
    return () => {
      ...
      disposed = true;
      removeUnlockListeners();
      if (!started) closeContext(context);
    };
  } catch {
    return () => undefined;
  }
