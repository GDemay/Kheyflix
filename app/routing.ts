...
export const parseRoute = (location = '/'): Route => {
  ...
  if (parts[0] === 'stream') return { section:'stream', id:parts[1], file:Number(parts[2]), title:url.searchParams.get('title') || 'Kheyflix video', compat:url.searchParams.get('compat')==='1' };
...
export const routePath = (route:Route) => {
  ...
  if (route.section === 'stream') return `/stream/${route.id}/${route.file}?title=${encodeURIComponent(route.title || 'Kheyflix video')}${route.compat?'&compat=1':''}`;
