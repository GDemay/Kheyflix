export type Route = { section: 'home'|'movies'|'series'|'search'|'title'|'watch'|'profile'|'library'|'debrid'|'stream'; id?: string; file?:number; title?:string; query?: string };
...
export const parseRoute = (location = '/'): Route => {
  ...
  if (parts[0] === 'debrid') return { section:'debrid', id:parts[1], file:Number(parts[2]), title:url.searchParams.get('title') || 'My Library title' };
  if (parts[0] === 'title') return { section:'title', id:parts[1] };
  if (parts[0] === 'profile') return { section:'profile' };
  if (parts[0] === 'library') return { section:'library' };
...
export const routePath = (route:Route) => {
  ...
  if (route.section === 'debrid') return `/debrid/${route.id}/${route.file}?title=${encodeURIComponent(route.title || 'My Library title')}`;
