...
export async function GET(request:Request,{params}:{params:Promise<{id:string;file:string}>}){
  try{
    ...
    const url=new URL(request.url);const start=Math.max(0,Number(url.searchParams.get('start')||0)),audio=Math.max(0,Math.floor(Number(url.searchParams.get('audio')||1))),video=url.searchParams.get('video')==='copy'?'&video=copy':'';const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101';const token=(url.searchParams.get('session')||crypto.randomUUID()).replace(/[^a-z0-9-]/gi,'');request.signal.addEventListener('abort',()=>{void fetch(`${base}/stop/${token}`,{method:'POST'}).catch(()=>undefined)},{once:true});const upstream=await fetch(`${base}/transcode/${id}/${file}?start=${start}&audio=${audio}&token=${token}${video}`,{cache:'no-store',signal:request.signal});
