...
export async function GET(request:Request,{params}:{params:Promise<{id:string;file:string}>}){
  try{
    ...
    const url=new URL(request.url);const start=Math.max(0,Number(url.searchParams.get('start')||0));const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101';const token=(url.searchParams.get('session')||crypto.randomUUID()).replace(/[^a-z0-9-]/gi,'');request.signal.addEventListener('abort',()=>{void fetch(`${base}/stop/${token}`,{method:'POST'}).catch(()=>undefined)},{once:true});const upstream=await fetch(`${base}/transcode/${id}/${file}?start=${start}&token=${token}`,{cache:'no-store',signal:request.signal});
