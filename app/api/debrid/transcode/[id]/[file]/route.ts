...
export async function GET(request:Request,{params}:{params:Promise<{id:string;file:string}>}){
  try{
    ...
    const url=new URL(request.url);const start=Math.max(0,Number(url.searchParams.get('start')||0)),audio=Math.max(0,Math.floor(Number(url.searchParams.get('audio')||1))),video=url.searchParams.get('video')==='copy'?'&video=copy':'',quality=new Set(['480','720','1080','original']).has(url.searchParams.get('quality')||'')?url.searchParams.get('quality'):'original';const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101';const token=(url.searchParams.get('session')||crypto.randomUUID()).replace(/[^a-z0-9-]/gi,'');request.signal.addEventListener('abort',()=>{void fetch(`${base}/stop/${token}`,{method:'POST'}).catch(()=>undefined)},{once:true});const upstream=await fetch(`${base}/transcode/${id}/${file}?start=${start}&audio=${audio}&token=${token}${video}&quality=${quality}`,{cache:'no-store',signal:request.signal});
...
export async function PATCH(request:Request){const token=(new URL(request.url).searchParams.get('session')||'').replace(/[^a-z0-9-]/gi,'');if(!token)return Response.json({error:{code:'SESSION_REQUIRED',message:'Playback session is required.'}},{status:400});const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101';await fetch(`${base}/touch/${token}`,{method:'POST'}).catch(()=>undefined);return new Response(null,{status:204})}
