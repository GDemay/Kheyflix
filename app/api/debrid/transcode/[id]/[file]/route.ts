...
const handlePatch=async(request:Request)=>{const token=(new URL(request.url).searchParams.get('session')||'').replace(/[^a-z0-9-]/gi,'');if(!token)return Response.json({error:{code:'SESSION_REQUIRED',message:'Playback session is required.'}},{status:400});const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101';await fetch(`${base}/touch/${token}`,{method:'POST'}).catch(()=>undefined);return new Response(null,{status:204})};
