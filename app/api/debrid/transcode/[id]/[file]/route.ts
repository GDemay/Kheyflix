...
const handleGet=async(request:Request,{params}:{params:Promise<{id:string;file:string}>})=>{
  ...
  try{
    ...
    const url=new URL(request.url),rawStart=Number(url.searchParams.get('start')||0),start=Number.isFinite(rawStart)?Math.max(0,rawStart):0,rawAudio=Number(url.searchParams.get('audio')||1),audio=Number.isFinite(rawAudio)?Math.max(0,Math.floor(rawAudio)):1,rawSubtitle=url.searchParams.get('subtitle'),subtitle=rawSubtitle!==null&&/^\d+$/.test(rawSubtitle)?`&subtitle=${rawSubtitle}`:'',video=url.searchParams.get('video')==='copy'?'&video=copy':'',quality=new Set(['bootstrap','480','720','1080','original']).has(url.searchParams.get('quality')||'')?url.searchParams.get('quality'):'original',rawSync=Number(url.searchParams.get('sync')||0),sync=Number.isFinite(rawSync)?Math.max(-5,Math.min(5,Math.round(rawSync*10)/10)):0;const base=process.env.KHEYFLIX_TRANSCODER_URL||'http://127.0.0.1:3101',requestedToken=(url.searchParams.get('session')||'').replace(/[^a-z0-9-]/gi,''),token=requestedToken||crypto.randomUUID();const upstream=await fetch(`${base}/transcode/${id}/${file}?start=${start}&audio=${audio}&sync=${sync}&token=${token}${subtitle}${video}&quality=${quality}`,{cache:'no-store',signal:request.signal});
