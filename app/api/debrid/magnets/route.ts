...
const failure = (request:Request,error:unknown) => {
  ...
  writeRequestLog(known.status >= 500 ? 'error' : 'warn','debrid.operation.failed',request,{code:known.code,status:known.status,error:error instanceof Error?error:new Error(String(error))});
...
const handlePost = async (request:Request) => {
  ...
  try {
    ...
    writeRequestLog('info','debrid.magnet.upload.completed',request,{magnetId:magnet.id,ready:Boolean(magnet.ready)});
